...
/* *******************************
  ...
  * **********************************/
  ...
  FirebaseStorage fireStorage =
      FirebaseStorage.instance; //instancia de firestorage para hacer peticiones
  ...
  Future uploadAndDeletePreviousFile(String destination, File file) async {
    try {
      ...
      await fireStorage
          .ref()
          .child(response.get("picture"))
          .delete()
          .whenComplete(() => log("imagen pasada" + " borrada con exito "))
          .catchError(
              (e) => log("Error al borran imagen pasada " + e.toString()));
  ...
  Future loadProfileImageOnStartup() async {
    try {
      ...
      log("Al momento de cargar la imagen en inicio de la app sucedió: " +
          e.toString());
    }
  }
  ...
  Future<String> getImageFromMessage(String messageId) async {
    final DocumentSnapshot<Object?> response =
        await questions.doc(messageId).get();
